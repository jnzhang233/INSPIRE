...
class INSPIRE_Learner:
    ...
    def _cauchy_probabilities(self, td_errors, visibility_matrix, temperature):
        """
        Compute share probabilities using a Cauchy (Lorentz) distribution.
        td_errors: (B, T, n_agents)
        visibility_matrix: (B, T, n_agents, n_agents)
        temperature: float, scale parameter gamma
        """
        batch_size, seq_len, n_agents = td_errors.shape
        # compute per-agent loc (median) and scale (temperature)
        # here we use median across time for loc
        loc = td_errors.median(dim=1, keepdim=True)[0]
        scale = torch.full_like(loc, temperature)

        probs_sum = torch.zeros_like(td_errors)
        for i in range(n_agents):
            # broadcast loc and scale to match shape
            loc_i = loc[:, :, i].unsqueeze(-1).expand(batch_size, seq_len, n_agents)
            scale_i = scale[:, :, i].unsqueeze(-1).expand(batch_size, seq_len, n_agents)
            # Cauchy PDF
            numerator = scale_i
            denominator = (td_errors - loc_i) ** 2 + scale_i ** 2
            cauchy_pdf = (1 / numpy.pi) * (numerator / denominator)
            # invert to emphasize extremes
            scores = -cauchy_pdf
            # zero-out non-visible
            visible = visibility_matrix[:, :, :, i].float()
            probs_sum += scores * visible
        # normalize
        probs = probs_sum / (probs_sum.sum(dim=1, keepdim=True) + 1e-8)
        return probs

    def _laplace_probabilities(self, td_errors, visibility_matrix, b_param):
        """
        Compute share probabilities using a Laplace distribution.
        td_errors: (B, T, n_agents)
        visibility_matrix: (B, T, n_agents, n_agents)
        b_param: float, scale parameter b
        """
        batch_size, seq_len, n_agents = td_errors.shape
        # compute per-agent loc (median)
        loc = td_errors.median(dim=1, keepdim=True)[0]
        b = torch.full_like(loc, b_param)

        probs_sum = torch.zeros_like(td_errors)
        for i in range(n_agents):
            loc_i = loc[:, :, i].unsqueeze(-1).expand(batch_size, seq_len, n_agents)
            b_i = b[:, :, i].unsqueeze(-1).expand(batch_size, seq_len, n_agents)
            laplace_pdf = (1 / (2 * b_i)) * torch.exp(-torch.abs(td_errors - loc_i) / b_i)
            scores = -laplace_pdf
            visible = visibility_matrix[:, :, :, i].float()
            probs_sum += scores * visible
        probs = probs_sum / (probs_sum.sum(dim=1, keepdim=True) + 1e-8)
    ...
    def ESR_with_priority(self,masked_q_td_error,visibility_matrix,t):
        ...
        if self.args.sending_test == 1:
            probabilities = self._cauchy_probabilities(
                masked_q_td_error.clone().detach(), visibility_matrix, temperature=self.args.probability_temperature)
        if self.args.sending_test == 2:
            probabilities = self._laplace_probabilities(
                masked_q_td_error.clone().detach(), visibility_matrix, b_param=self.args.probability_temperature)
