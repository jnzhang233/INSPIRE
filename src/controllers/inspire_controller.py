...
class INSPIRE_MAC:
    ...
    def forward_score(self, ep_batch, t, score, test_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t)
        avail_actions = ep_batch["avail_actions"][:, t]
        agent_weight = score.permute(0, 2, 1)
        agent_weight = th.sum(agent_weight,dim=2)
        agent_outs, self.hidden_states = self.agent.forward_score(agent_inputs,agent_weight, self.hidden_states)
