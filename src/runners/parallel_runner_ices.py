...
class ParallelRunner:
    def __init__(self, args, logger):
        ...
        self.env = env_fn #给ices_run.py传一下，方便那边调个函数
        self.ps = []
    ...
    def run(self, test_mode=False):
        ...
        while True:
            ...
            if save_probs:
                ...
                actions, log_info = self.mac.select_actions(
                    self.batch,
                    t_ep=self.t,
                    t_env=self.t_env,
                    bs=envs_not_terminated,
...
def env_worker(remote, env_fn):
    ...
    while True:
        ...
        if cmd == "step":
            ...
            state = env.get_state_ICES()
            avail_actions = env.get_avail_actions()
            obs = env.get_obs_ICES()
            remote.send(
                {
